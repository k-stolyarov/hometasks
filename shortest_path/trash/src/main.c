...
//			radius, radius, 0, 360 * 64);
//
//		for (i = 0; i < path.pointNumber - 1; i++)
//		{
//			p1 = g->points[path.points[i]];
//			p2 = g->points[path.points[i + 1]];
...
int main(int argc, char **argv)
{
  ...
  /* and now it starts: the event loop */
	...
	//		draw(&triangles, draw_first_point, draw_path,p1, p2, &_path);
	//		break;
	...
	//				if (!IsInsideTriangle(p1, &triangles) == TRUE)
	//				{
	...
	//			{
	//				p2.x = report.xbutton.x;
	//				p2.y = report.xbutton.y;

	//				if (!IsInsideTriangle(p2, &triangles) == TRUE)
	//				{
	//					printf("(%d %d) (%d %d)\n", p1.x, p1.y, p2.x, p2.y);
